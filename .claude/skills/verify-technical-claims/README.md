...
This repository contains a skill definition for pre-empirically verifying technical claims. The skill is designed to assess technical concepts, physics claims, or market assertions by prioritizing evidence from reliable sources.

## Repository contents

- `.claude/skills/verify-technical-claims/SKILL.md` — main skill description, core principles, reasoning workflow, and response format.
- `agents/` — skill agent configurations.
- `evals/` — evaluation scenarios for testing skill quality.
...
4. Provide answers with proportional truth status and recommended next actions.

## How to use

1. Read `.claude/skills/verify-technical-claims/SKILL.md` to understand the skill's principles, workflow, and answer format.
...
4. Run evaluations with `evals/` to verify the skill's consistency when applicable.

## Contribution

- Add new test cases to `evals/` for additional scenarios.
- Update `references/` with new protocols, frameworks, or guidance.
- Revise `.claude/skills/verify-technical-claims/SKILL.md` if the verification process or response style needs adjustment.
